...
public class DefaultWebService implements WebService {
    ...
    private static MetricService.MetricInvalidMsgReasonType convertToMetricCategory(
        ...
        return switch (errorCode.getCodeEnum()) {
            case C2SIM_MSG_SIZE_EXCEEDED,
                 ...
                 SHARED_SESSION_ALREADY_EXIST -> MetricService.MetricInvalidMsgReasonType.BAD_REQUEST;
            case C2SIM_MSG_DECODING_ERROR,
                 ...
                 C2SIM_SCHEMA_NOT_SUPPORTED -> MetricService.MetricInvalidMsgReasonType.INVALID_FORMAT;
    ...
    public C2SimClaimsBuilder getClaimsBuilder() throws AuthorisationException {
        ...
        try {
            ...
            logger.error("Unable to validate bearer,  " +
                    "the C2SIM server failed to retrieve public key of keycloak (needed for signature validation)");
            throw new AuthorisationException(SIGNATURE_VERIFICATION_FAILED,
                    "C2SIM server was not able to verify signing of bearer: " +
                            ex.getMessage());
        }
        return this.claimsBuilder;
    }
    ...
    private void handleAuthorizationException(AuthorisationException e, Context ctx) {
        ...
        var sharedSession = ContextHelper.getAttributeValue(ctx, ATTRIB_SHARED_SESSION_ID);
        var clientId = ctx.header(ContextHelper.ATTRIB_HEADER_PARAM_CLIENT_ID);
        var ipAddress = ctx.ip();
        logger.error("Shared session '{}': " +
                        "Return REST STATUS 401 (no auth) for client '{}' ({}): {} [{}] tracking ID '{}'",
                sharedSession,
                clientId,
                ipAddress,
                e.getMessage(),
                ...
                trackingId);
        // Update metrics
        metricService.incAuthFailed(ipAddress);
        // Update audit log
        auditService.authorizationFailure(
                e.getErrorCode(),
                e.getMessage(),
                trackingId,
                sharedSession,
    ...
    private void handleRESTFulC2SimRestException(C2SimException c2SimRestException, Context ctx) {
        ...
        var auth = ContextHelper.getAuthorizer(ctx); // Just to check if auth info is available, not used for now
        var header = ContextHelper.getC2SimHeader(ctx);

        // Get SYSTEM NAME of invoker
        String systemName = SystemNameHelper.getSystemName(auth, header);


        updateMetricsForFailedRequests(c2SimRestException, sharedSessionName, systemName, ctx.ip());
        logger.error(
    ...
    private void updateMetricsForFailedRequests(
            C2SimException c2SimRestException,
            String sharedSessionName,
            String systemName,
            String ipAddress) {
        Objects.requireNonNull(c2SimRestException);
        ...
        try {
            ...
            metricService.incInvalidMessagesSendByC2SimClient(
                    sharedSessionName, ipAddress, systemName, convertToMetricCategory(errorCode));
        } catch (Exception e) {
    ...
    public void serve() {
        ...
        String url = "http://" + externalHostName + ":" + this.webServerPort;
