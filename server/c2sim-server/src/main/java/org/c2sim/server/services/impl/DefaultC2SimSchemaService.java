...
public class DefaultC2SimSchemaService implements C2SimSchemaService {
  ...
  private final Map<String, SchemaInfo> schemaVersionFolders =
      new HashMap<>(); // schema version -> folder path
  ...
  private static XsdNamespaceInfo[] extractNamespacesFromFolder(File folder) {
    ...
    File[] files = folder.listFiles((dir, name) -> name.toLowerCase().endsWith(".xsd"));
  ...
  private void initializeXsdFolder() {
    logger.info(
        "The C2SIM server uses LOX library which is build (JAXB) against XSD namespace '{}' ",
  ...
  public LoxXsdValidator validate(String schemaVersion, InputStream xmlStream)
    ...
    if (info != null) {
      return LoxXsdValidator.doValidation(
          schemaVersion, info.xsdFiles[0].fileName, ns -> null, xmlStream);
  ...
  private void registerSchema(String version, Path folder) {
    ...
    if (count != 1) {
      logger.warn(
          "The folder '{}' must contain exactly one XSD definition, skip version '{}'.",
          folder,
          version);
      return;
    }
    ...
    var namespaceInOneLine =
        String.join("|", Arrays.stream(xsdFiles).map(x -> x.namespace).toList());
  ...
  private record SchemaInfo(String version, XsdNamespaceInfo[] xsdFiles, Path folder) {}

  private record XsdNamespaceInfo(URL fileName, String namespace) {}
}
